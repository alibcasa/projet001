...
import { discoverJoplin, joplinAuthCheck, joplinAuthStart } from '@/lib/joplin'
...
export async function POST() {
  ...
  if (!baseUrl) return NextResponse.json({ error: 'Joplin Desktop introuvable. Activez Web Clipper dans Joplin.' }, { status: 503 })
...
export async function GET() {
  ...
  if (data?.enabled && config.baseUrl && config.token) return NextResponse.json({ status: 'connected', baseUrl: config.baseUrl })
